...
export function CommerceDashboard() {
  ...
  return <div className="relative min-h-screen overflow-x-hidden bg-background text-foreground">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 w-[17.5rem] border-r border-border bg-sidebar p-4 shadow-[24px_0_80px_rgba(0,0,0,0.12)] backdrop-blur-2xl transition-transform lg:translate-x-0 ${mobileNav ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex h-14 items-center gap-3 px-3"><div className="flex size-10 items-center justify-center rounded-xl border border-primary/25 bg-primary/10 text-base font-bold text-primary">R</div><div><p className="text-base font-semibold leading-5 tracking-tight">Raingentic</p><p className="mt-0.5 text-sm text-muted-foreground">PreFlight</p></div></div>
    ...
    <div className="relative z-10 lg:pl-[17.5rem]">
      <header className="sticky top-0 z-20 flex h-[4.5rem] items-center justify-between border-b border-border bg-background/75 px-4 backdrop-blur-2xl md:px-8"><div className="flex items-center gap-3"><Button variant="ghost" size="icon" className="lg:hidden" aria-label="Open navigation" onClick={() => setMobileNav(true)}><Menu /></Button><p className="text-sm font-medium text-muted-foreground">{world === "buying" ? "Buying" : "Selling"}</p><ChevronRight className="size-4 text-muted-foreground" /><p className="text-sm font-medium text-foreground">{demoMode ? "Prepared demo" : pageTitles[page]}</p></div><div className="flex items-center gap-3"><Badge variant="outline" className="hidden sm:inline-flex">Sandbox</Badge>{demoMode ? <Button size="sm" variant="outline" onClick={() => setDemoMode(false)}>Exit demo</Button> : <Button size="sm" onClick={() => setDemoMode(true)}><Play />Run demo</Button>}</div></header>
      ...
      <main className="mx-auto max-w-[88rem] p-5 md:p-8 lg:p-10">
        {demoMode ? <PresentationMode /> : <>
          ...
          {world === "buying" && page === "organization" && <Organization company={company} setCompany={setCompany} saved={saved} setSaved={setSaved} readiness={readiness} />}
          ...
          {world === "selling" && page === "organization" && <Organization company={company} setCompany={setCompany} saved={saved} setSaved={setSaved} readiness={readiness} />}
        </>}
      </main>
    </div>
  </div>;
...
function Organization({ company, setCompany, saved, setSaved, readiness }: { company: { name: string; website: string; approvalThreshold: string }; setCompany: (value: { name: string; website: string; approvalThreshold: string }) => void; saved: boolean; setSaved: (value: boolean) => void; readiness: Record<string, boolean> }) {
  return <Section title="Organization" description="Company identity, infrastructure connections, and shared commerce policy."><div className="grid gap-5 lg:grid-cols-[1fr_1.3fr]"><Card><CardHeader><CardTitle>Company profile</CardTitle></CardHeader><CardContent className="space-y-5"><div className="space-y-2"><Label htmlFor="company-name">Company name</Label><Input id="company-name" value={company.name} onChange={event => { setCompany({ ...company, name: event.target.value }); setSaved(false); }} /></div><div className="space-y-2"><Label htmlFor="company-site">Website</Label><Input id="company-site" value={company.website} onChange={event => { setCompany({ ...company, website: event.target.value }); setSaved(false); }} /></div><div className="space-y-2"><Label htmlFor="approval-threshold">Approval threshold in US dollars</Label><Input id="approval-threshold" type="number" value={company.approvalThreshold} onChange={event => { setCompany({ ...company, approvalThreshold: event.target.value }); setSaved(false); }} /></div>{saved && <div className="rounded-lg border bg-secondary/50 p-4 text-sm">Organization settings saved for this session.</div>}</CardContent><CardFooter><Button onClick={() => setSaved(true)}>Save organization</Button></CardFooter></Card><Card><CardHeader><CardTitle>Infrastructure</CardTitle><CardDescription>Connections used by this organization. Credentials and payment keys remain server-side.</CardDescription></CardHeader><CardContent className="space-y-3">{[[CreditCard,"Rain","Cards, accounts, routes, and treasury",readiness.rain],[CircleDollarSign,"Monad","x402 settlement",readiness.monad],[Store,"A2A","Discovery and negotiation",readiness.a2a],[FileCheck2,"AP2","Signed payment authorization",readiness.ap2]].map(([Glyph,name,description,ready]) => { const Icon = Glyph as typeof CreditCard; return <div key={String(name)} className="flex items-center justify-between gap-4 rounded-lg border p-4"><div className="flex items-center gap-3"><Icon className="size-5 text-muted-foreground" /><div><p className="font-medium">{String(name)}</p><p className="mt-1 text-sm text-muted-foreground">{String(description)}</p></div></div><Badge variant={ready ? "secondary" : "outline"}>{ready ? "Connected" : "Setup"}</Badge></div>; })}</CardContent></Card></div></Section>;
}
