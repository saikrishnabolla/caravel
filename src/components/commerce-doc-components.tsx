...
export function CatalogSetup({ onLoadDemo, onPublished }: { onLoadDemo?: () => void; onPublished?: (publicationUrl: string) => void }) {
  ...
  async function connectSource() {
    ...
    try {
      ...
      const document = await response.json() as { error?: string; title?: string; operations?: Array<{ method: string; path: string; name: string; price?: { amount?: string } | null }>; products?: Array<{ id: string; name: string; vendor: string; type: string; variants: Array<{ id: string; name: string; price: string; available: boolean }> }> };
      if (!response.ok) throw new Error(document.error ?? "Catalog import failed");
      if (source === "openapi") {
        setOperations((document.operations ?? []).map(operation => [operation.method,operation.path,operation.name,`$${operation.price?.amount ?? "0.01"}`]));
  ...
  function updateOperationPrice(index: number, value: string) {
    setOperations(current => current.map((operation, operationIndex) => operationIndex === index ? [operation[0],operation[1],operation[2],`$${value}`] : operation));
